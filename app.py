...
maringa_file = os.path.join(application_path, 'codigos_cianorte.xlsx')
tapejara_file = os.path.join(application_path, 'codigos_umuarama.xlsx')

wb_debitos_maringa = openpyxl.load_workbook('codigos_cianorte.xlsx')
sheet_debitos_maringa = wb_debitos_maringa['Planilha1']

wb_debitos_tapejara = openpyxl.load_workbook('codigos_umuarama.xlsx')
...
def ler_progresso_maringa():
    if os.path.exists("progresso_cianorte.txt"):
        with open("progresso_cianorte.txt", "r") as file:
            return int(file.read().strip())
    return 2  # Se não houver progresso registrado, começa da linha 2

# Função para salvar o progresso
def salvar_progresso_maringa(linha):
    with open("progresso_cianorte.txt", "w") as file:
        file.write(str(linha))


def ler_progresso_tapejara():
    arquivo_progresso = "progresso_umuarama.txt"
...
def salvar_progresso_tapejara(linha):
    with open("progresso_umuarama.txt", "w") as file:
        file.write(str(linha))
...
def pegar_debitos_maringa(linha_inicial_maringa):
    ...
    driver.get('https://cianorte.oxy.elotech.com.br/portal-contribuinte/consulta-debitos')
    ...
    for linha in sheet_debitos_maringa.iter_rows(min_row=linha_inicial_maringa, max_row=70):
            ...
            wb_resultado.save('empresas_sem_debitos_cianorte.xlsx')
            ...
            try:
                ...
                folder_path = r'C:\Users\Logika\Desktop\Puleman'
...
def pegar_debitos_tapejara(linha_inicial_tapejara):
    ...
    driver = webdriver.Chrome()
    driver.get('https://servicos.umuarama.pr.gov.br/portal-contribuinte/consulta-debitos')
    ...
    for linha in sheet_debitos_tapejara.iter_rows(min_row=linha_inicial_tapejara, max_row=100):
            ...
            wb_resultado.save('empresas_sem_debitos_umuarama.xlsx')
...
img_maringa = ctk.CTkImage(Image.open("pref.cianorte.png"), size=(300, 100))
...
entry_maringa = ctk.CTkEntry(
    frame_maringa, 
    placeholder_text="Digite a linha inicial do Excel para Cianorte", 
    font=("Helvetica", 14), 
    width=300
)
...
img_tapejara = ctk.CTkImage(Image.open("pref.umuarama.png"), size=(300, 100))
...
entry_tapejara = ctk.CTkEntry(
    frame_tapejara, 
    placeholder_text="Digite a linha inicial do Excel para Umuarama", 
    font=("Helvetica", 14), 
    width=300
)
